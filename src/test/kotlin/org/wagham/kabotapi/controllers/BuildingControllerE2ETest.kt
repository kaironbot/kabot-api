...
suspend fun StringSpec.buildingE2ETest(
    url: String,
    client: HttpClient,
    guild: String,
    ...
    "Can get all the Buildings" {
        ...
        val backgrounds = objectMapper.readValue(response.body(), object : TypeReference<List<BuildingWithBounty>>(){})
        response.statusCode() shouldBe 200
        backgrounds.size shouldBeGreaterThan 0
    }

    "Requesting the Buildings from a non-existing guild result in 404" {
    ...
    "Requesting the Buildings with no Guild ID should result in 400" {
        val request = HttpRequest.newBuilder()
            .uri(URI.create(url))
            .GET()
            .build()
